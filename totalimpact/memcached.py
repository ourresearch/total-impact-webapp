...
# from https://github.com/openstack/swift/blob/master/swift/common/memcached.py
...
class MemcacheRing(object):
    ...
    def _exception_occurred(self, server, e, action='talking',
        ...
        if isinstance(e, Timeout):
            logging.error("Timeout %(action)s to memcached: %(server)s" %{'action': action, 'server': server})
        else:
            logging.exception("Error %(action)s to memcached: %(server)s" %{'action': action, 'server': server})
        try:
            if fp:
                fp.close()
                del fp
